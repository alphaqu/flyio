...
/**
 * A super fact IO implementation to use with Bytebuffers or directly. Always call .close if you allocate with off-heap.
 */
...
public final class UnsafeIO implements IOInterfrace {
	 ...
	 * If this address is from a bytebuffer gc might clear them and segfault the jvm.
	 */
	public static final UnsafeIO wrap(final long address) {
		return new UnsafeIO(address, null);
	}
	...
	public void setPos(int pos) {
		this.currentAddress = address + pos;
	}
	...
	public static final int getStringBytes(String string) {
		return (((byte[]) UNSAFE.getObject(string, STRING_FIELD_OFFSET)).length);
	}
}
